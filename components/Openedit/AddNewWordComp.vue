<template>
    <form @submit.prevent="submitForm" id="openeditword" class="row g-3">
        <div class="col-md-4">
            ...
            <input type="text" class="form-control form-control-sm word" maxlength="50" :disabled="disabled" required name="word" :value="data.word">
        </div>
        <div class="col-md-4">
...
const addSentence = () => {
    ...
    /*
    if ( data.sentences.length < 11) {
        data.sentences.push({'en':'','zh':''})
    }

    if ( data.sentences.length == 10 ) { addmore.value = false }
    */
}
</script>
